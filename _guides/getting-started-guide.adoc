...
The easiest way to create a new {project-name} project is to open a text editor in the `getting-started` folder and to edit a config file.
Browse the following content of `conf/parse-apache-logs.yml` in any text editor if you want to make some adjustments.
...
Let’s start with a single broker instance. In the directory directory `getting-started` create your docker-compose.yml with the following content:
...
Now you can now run the job inside the logisland container

`sudo docker exec -ti getting-started_logisland_1 ./bin/logisland.sh --conf conf/parse-apache-logs.yml`
...
[Tip]
  ...
  `sudo docker exec -ti getting-started_kafka_1 sh -c "cat /tmp/data/access.log | /opt/kafka/bin/kafka-console-producer.sh --broker-list kafka:9092 --topic logisland_raw"`

If you want to see what is flowing into `logisland_raw` topic, just run the following :

`sudo docker exec -ti getting-started_kafka_1 /opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server kafka:9092 --topic logisland_raw`
...
`sudo docker exec -ti getting-started_kafka_1 /opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server kafka:9092 --topic logisland_events`
...
Another metric is processing time, which give us here about 35ms for 
