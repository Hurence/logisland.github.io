...
* The solution is located in the `conf/rest/uppercase_input_with_openfaas_async.yml` file.
...
----
    - controllerService: faas_service_rest
      ...
      configuration:
        ...
        record.deserializer.for.body: none
      ...
      processorConfigurations:
        ...
        - processor: defect_tagger
          ...
          configuration:
            ...
            field.http.response: http_query_uppercase
            request.method: "POST"
            request.mime.type: "plain/text"
            request.body: "${http_query}"
            keep.only.response.body: true
----
...
The body will be the field http_query of the records as specified by property `request.body` which is a String, that is why we specified request
mime type as plain text. Then the response is stored in the field `http_query_uppercase` of the input records as a String as no serializer is specified in the service 'none'.

Normally the response is stored as a record, the record returned by the Service. It contains the http code received, the http message and the http body. The response body is deserialized
by the specified deserializer in the service. Bot as we specified conf `keep.only.response.body: true`, we only store the field of the body response which in our case is a String (no deserializer).
...
differently you will have to chain other processors like the FilterRecords processor. Otherwise you can use the property `valid.http.response` (See processor conf).
...
----
{
  "id" : "ed72081d-89be-4f08-b3cf-20f0948a464c",
  "type" : "apache_log",
  "creationDate" : 1570519094000,
  "fields" : {
    "record_value" : "123.124.125.126 - - [08/Oct/2019:09:18:14 +0200] \"POST /app/main/posts HTTP/1.0\" 301 5070",
    "identd" : "-",
    "http_version" : "HTTP/1.0",
    "record_type" : "apache_log",
    "src_ip" : "123.124.125.126",
    "record_id" : "ed72081d-89be-4f08-b3cf-20f0948a464c",
    "http_method" : "POST",
    "http_query" : "/app/main/posts",
    "bytes_out" : "5070",
    "http_query_uppercase" : "/APP/MAIN/POSTS",
    "function_name" : "uppercase",
    "http_status" : "301",
    "record_time" : 1570519094000,
    "user" : "-"
  }
}
----

We can see here that field `http_query_uppercase` is indeed in uppercase.
